...
def loaddocuments():
    ...
    df = pd.read_excel('./path_module.xlsx')
    target = np.zeros((df.count()['MODULENAME'],), dtype=np.int64)
    #df = df[(df['MODULENAME'] == 'SR_FRAME_DRV') | (df['MODULENAME'] =='RTDEVM_PIC') |
    ...
    return sklearn.datasets.base.Bunch(data=documents, target=target, target_names=categories) 
